...
def run_length_plot(length_input,outdir,out_prefix,args):
    ...
    chrom_split = tel_length_tidy["chrom"].str.partition("_")
    tel_length_tidy["chr"] = chrom_split[0]
    tel_length_tidy["haplotype"] = chrom_split[2].replace("", "hap0")

    # Filter out mitochondrial chromosomes
    mito_regex = r'(?i)^(M|MT|Mito)$'
    is_mito = tel_length_tidy['chr'].str.match(mito_regex).fillna(False)
    tel_length_tidy = tel_length_tidy[~is_mito]
    ...
    is_haplotype_data = tel_length_tidy['chrom'].str.contains('_').any()
