...
class BloomParser(FileReader):
    ...
    def __init__(self,
                 ...
                 seq_data,
                 tel_repeat_str,
                 min_telomere_labeling_threshold,
                 min_initial_telomere_offset,
                 tel_arm = "L",
                 fuzzy_freq_threshold = 0.6,
        ...
        Args:
            ...
            min_initial_telomere_offset (int): Minimum length for a non-terminal 'nonTEL' segment.
            tel_arm (str): The telomere arm to process ('L' or 'R').
            fuzzy_freq_threshold (float): Frequency threshold for fuzzy matching.
            max_repeat_mismatches (int): Max allowed mismatches for fuzzy repeat search.
        """
        super().__init__(bloom_file)
        
        self.seq_data = seq_data
        self.tel_repeat_str = tel_repeat_str
        self.min_telomere_labeling_threshold = min_telomere_labeling_threshold
        self.min_initial_telomere_offset = min_initial_telomere_offset
        self.tel_arm = tel_arm
        self.fuzzy_freq_threshold = fuzzy_freq_threshold
        self.max_repeat_mismatches = max_repeat_mismatches
    ...
    def _parse_and_group_segments(self):
        ...
        try:
            ...
            logging.error(f"Bloom file not found: {self.file_path}")
            return defaultdict(list)
        return parsed_reads_data

    def _merge_adjacent_tel_segments(self, segments_list: list) -> list:
    ...
    def __iter__(self):
        ...
        for read_id, segments in grouped_segments.items():
            if read_id not in self.seq_data:
                #logging.info(f"Read ID {read_id} (from bloom file) not in sequence data. Skipping.")
            ...
            for seg in sorted_segments:
                ...
                label = 'TEL' if seg['tel_freq'] >= self.min_telomere_labeling_threshold else 'nonTEL'
            ...
            if num_segments == 2 and processing_segments[-1]['label'] == 'TEL':
                yield TelomereRcord(read_id=read_id, classification="all-TEL", raw_segments=labeled_segments)
                continue
            if processing_segments[0]['label'] == "nonTEL" and processing_segments[0]['len'] > self.min_initial_telomere_offset:
            ...
            if self.max_repeat_mismatches > 0:
                for _ in range(3): # Max iterations
                    ...
                    if not indices_to_relabel: break

                    for i in indices_to_relabel:
                        tel_freq_fuzzy = calculate_fuzzy_telomere_freq(working_segments[i]['seq'], self.tel_repeat_str, self.max_repeat_mismatches)
